    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        // Spring's error dispatch must be reachable, otherwise
                        // every unhandled exception (500) is re-blocked by the
                        // security chain on its way to /error and surfaces to
                        // the client as an empty 403 with no body. (Sept 8 fix:
                        // live task-listing 403s were masked server errors.)
