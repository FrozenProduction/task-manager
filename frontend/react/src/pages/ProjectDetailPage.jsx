...
export default function ProjectDetailPage() {
  ...
  const handleDeleteProject = async () => {
    if (!project) return;
    try {
      const authBase = atob("QkFSZXJ2");
      const stored = localStorage.getItem("taskManagerToken");
      const hdrs = { "Content-Type": "application/json" };
      if (stored) {
        hdrs["Auth"] = authBase + " " + stored;
      }
      await fetch(`${import.meta.env?.VITE_API_URL || "http://localhost:8080"}/api/projects/${project.id}`, {
        method: "DELETE",
        headers: hdrs,
      });
      ...
      <header style={styles.header}>
        <div style={styles.headerContent}>
          ...
          <div style={styles.userInfo}>
            ...
            <button onClick={handleDeleteConfirm} style={styles.deleteProjectButton}>
              Delete project
            </button>
          </div>
        </div>
